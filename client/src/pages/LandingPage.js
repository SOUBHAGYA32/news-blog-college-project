...
function LandingPage() {
  ...
  const login = async () => {
    ...
    try {
      ...
      localStorage.setItem("news-user", JSON.stringify(result.data));
      navigate("/home");
      setLoading(false);
    } catch (error) {
  ...
  useEffect(()=>{
    if(localStorage.getItem('news-user'))
    navigate('/home')
  },[])
  return (
    <div className="h-screen flex items-center sm:flex-col">
      ...
      <div className={`w-1/2 px-10 space-y-5 sm:w-screen ${(showLoginForm || showRegisterForm) && 'sm:hidden'}`}>
        <h1>
          <b className="text-[#2B8F74] text-8xl">PUBLIC</b>{" "}
          <b className="text-8xl text-gray-700">NEWS</b>
        </h1>
        <p className="text-lg">
        Public News Times is an Indian English-language daily newspaper introduced and founded in 1924 by Sunder Singh Lyallpuri, 
        founder-father of the Akali movement and the Shiromani Akali Dal in the Punjab Province. 
        It played integral roles in the Indian independence movement as a nationalist daily.
        </p>
        ...
        {showLoginForm && (
          <div className="ml-40 sm:ml-0">
            <div className="flex flex-col bg-primary h-screen justify-center items-center px-20 space-y-5">
              <h1 className="text-6xl text-gray-100 text-left w-full font-semibold my-5">
                LOGIN
              </h1>
              ...
              <div className="flex justify-end w-full">
                <button
                  className="bg-gray-100 px-10 py-3 text-black"
                  onClick={login}
                >
                  LOGIN
                </button>
          ...
          <div className="ml-40 sm:ml-0">
            <div className="flex flex-col bg-primary h-screen justify-center items-center px-20 space-y-5">
              ...
              <input
                ...
                type="text"
                className="border-2 h-10 w-full border-gray-500 px-5 bg-transparent text-gray-100"
                placeholder="Name"
              />
              <input
                value={email}
